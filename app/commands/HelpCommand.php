...
class HelpCommand implements Command
{
	 ...
	 */
	function __construct(Output $output, Nette\DI\Container $container)
	{
		$this->output = $output;
	...
	{
		$commands = array();
		foreach ($this->container->findByType('Taco\Commands\Command') as $name) {
			$commands[] = $this->formatCommand($this->container->getService($name));
		}
...
");
	...
	private function formatCommand($command)
	{
		...
		return sprintf("  %-10s %s\n%s", $command->getName(), $command->getDescription(), implode(PHP_EOL, $options));
	}
